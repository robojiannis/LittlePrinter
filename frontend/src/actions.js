...
export async function sendImageToPrinter(dispatch, printer, payload) {
  console.log("sendImageToPrinter", printer, payload);
  
  dispatch({ type: "sending", printer, payload });
  try {
    // If payload is a string (from poster-font), send it as text
    if (payload.type === 'poster-font') {
      await api.sendImage({ image: payload.image, url: printer.url });
    } else if (payload.type === 'dithergram') {
      // If payload is an object with image and caption (from dithergram)
      await api.sendImage({ 
        url: printer.url, 
        image: payload.image,
    ...
    dispatch({ type: "success", printer, payload });
  } catch (err) {
    console.error(err);
    dispatch({ type: "failure", printer, payload });
    throw err;
  }
}
